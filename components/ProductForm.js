...
export default function ProductForm({
  ...
  useEffect(() => {
    if (isExclusive) {
      setStock(1);
      setCategory(""); // ukloni kategoriju ako je već selektovana
    }
  }, [isExclusive]);
  
  useEffect(() => {
    setIsExclusive(!!existingIsExclusive);
  }, [existingIsExclusive]);
  

  useEffect(() => {
    console.log("Trenutni URL-ovi slika:", images);
  }, [images]);
  ...
  async function saveProduct(ev) {
    ...
    const data = {
      ...
      stock,
      images,
      category: isExclusive ? null : category,
  ...
  function updateImagesOrder(newOrder) {
    setImages([...newOrder]); // Pravilno ažuriranje state-a
  }
  ...
  className={isExclusive ? "bg-gray-200 text-gray-500 cursor-not-allowed" : ""}
>
...
/>
  ...
  <input
    ...
    id="isExclusive"
    checked={isExclusive}
    onChange={(ev) => setIsExclusive(ev.target.checked)}
    className="w-4 h-4"
  />
  <label htmlFor="isExclusive" className="text-sm text-gray-700 font-medium">
